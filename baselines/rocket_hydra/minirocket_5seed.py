...
OUTPUT_ROOT = Path(__file__).resolve().parent.parent / "outputs" / "MiniRocket"
...
def run_one(dataset_name: str, seed: int):
    ...
    row = make_base_result_row(
        "MiniRocket", dataset_name, seed, X_train, y_train, X_test, y_test,
...
def main():
    ...
    rows = get_existing_rows(OUTPUT_ROOT / "MiniRocket_per_seed_results.csv")

    for dataset_name in DATASETS:
        for seed in SEEDS:
            print("\n" + "=" * 90, flush=True)
            print(f"MiniRocket | dataset={dataset_name} | seed={seed}", flush=True)
            if SKIP_COMPLETED and row_already_ok(rows, "MiniRocket", dataset_name, seed):
            ...
            try:
                ...
                traceback.print_exc()
                row = {"model": "MiniRocket", "dataset": dataset_name, "seed": int(seed), "status": "ERROR", "error": repr(e)}
            rows = remove_same_key(rows, "MiniRocket", dataset_name, seed)
            rows.append(row)
            save_experiment_tables(rows, OUTPUT_ROOT, "MiniRocket")

    print("\nDone. Use MiniRocket_summary_mean_std.csv for the paper table.", flush=True)
